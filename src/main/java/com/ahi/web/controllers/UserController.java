...
public class UserController {
	...
	public ResponseEntity<List<UserModel>> getUsers(Principal principal) {
	...
	public ResponseEntity<UserModel> getLoggedInUserDetails(Principal principal) {

		return new ResponseEntity<UserModel>(userService.getUser(principal.getName()), HttpStatus.OK);
	}

	@RequestMapping(value = "/{username}", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<UserModel> getUser(@PathVariable String username, Principal principal) {
	...
	@ResponseBody
	public ResponseEntity<?> saveUser(@RequestBody UserModel user, Principal principal) throws Exception {
	...
	@ResponseBody
	public ResponseEntity<UserModel> saveUser(@RequestBody UserModel userModel, Principal principal, @PathVariable String userId)
	throws Exception {
		try {
			if (userModel.getId() == null || userModel.getId() == 0)
				userModel = userService.addUser(userModel, userId);
			else
