...
public class TaskController {
	...
	@RequestMapping(value = "/all/{empId}", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<List<TasksModel>> findAllTask(@PathVariable Integer empId) {

		try {
			List<TasksModel> models = tasksService.getAllTasks(empId);
			return new ResponseEntity<List<TasksModel>>(models, HttpStatus.OK);
		} catch (AHCustomException e) {
			throw new WebServerException(e.getErrorMessage(), e);
		}
